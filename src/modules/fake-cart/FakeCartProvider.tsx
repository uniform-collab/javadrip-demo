...
const FakeCartContextProvider: FC<FakeCartContextProviderProps> = ({ styles, children }) => {
  ...
  const fetchProductCart = useCallback(async () => {
    ...
    const cartProducts = Object.entries(cart).map(([productKey]) => productKey);
    const productCodes = encodeURIComponent(JSON.stringify(cartProducts));
    const productsResponse = await fetch(`/api/cart?productKeys=${productCodes}&locale=${locale}`)
      .then(res => res.json())
      .finally(() => setIsCartLoading(false));

    const newCartItems = productsResponse.map((product: CommerceTypes.Product) => {
  ...
  const addItemToFakeCart = useCallback(
    (item: CommerceTypes.FakeCartAddItem) => {
      const cartItem = cart[item.productKey];
  ...
  const cartAmount = useMemo(
    () => cartItems.reduce((acc, cartItem) => acc + cartItem.quantity * ((cartItem.product.price as number) || 0), 0),
    [cartItems]
  );
